-- Notes for 'Quantifiers and Equality'

-- Namespacing the entire file 
-- avoids conflicts with other notes and solutions
namespace quantifiers_and_equality_notes
...
-- We use the `\|` symbol instead of `\mid` ('∣') or ASCII `|` since they are already defined in the standard library
infix:51 " ‖ " => divides 
...
end quantifiers_and_equality_notes 
